...
$access_token = '';
    ...
    $redirect_uri = 'http://' . $domain . '/msg_test_alpha.php';
    $api_url = 'http://' . $domain . '/msg_test_api_alpha.php';
    if (isset($_GET['code'])) {
        $code = $_GET['code'];
        $state = $_GET['state'];
    ...
    function getAccessToken($code, $state, $redirect_uri) {
        $url = 'https://alpha-kauth.kakao.com/oauth/token';
        $params = [
        ...
        if (isset($result['access_token'])) {
          ...
          //redirect to msg_test.php
            header('Location: msg_test_alpha.php');
            exit;
        }
    }
    ...
    function getAppkey($type) {
        switch($type) {
            case 'none':
                return 'df9841d3b0e8d27f970be82361627da6';
            case 'web_only':
                return '04f07bb81d2739622b6afd81155a13d1';
            case 'android_only':
                return '7a7192f4e06e56073d135d81ab7bffda';
            case 'all':
                return 'fdead3b15231640d4266d9f4046a9a65';
        }
    }
...
<head>
    <meta charset="UTF-8">
    <title>(Alpha)메시지 링크 테스트</title>
    <!-- 카카오 SDK 추가 -->
    <script src="https://alpha-developers.kakao.com/sdk/js/kakao.min.js"></script>
</head>
<body>
    <h1>(Alpha)메시지 링크 테스트(메시지API는 로그인 먼저해야 앱 변경됨)</h1>
...
<!--플랫폼 정보 Android Only-->
            <tr>
                <td rowspan="4">플랫폼 정보 Native Only<button class="kakao-share-btn" onclick="loginWithKakao('android_only')">카카오로그인</button> <input type="text" value="<?=getAccessTokenRtn('android_only'); ?>" id="access_token"/> </td>
...
<tr>
            ...
            <tr>
                ...
                <td> 웹으로 보기 버튼 표기, <br/> 앱으로 보기 버튼 없음 </td>
                <td> 웹으로 보기 버튼 표기 <br/> 앱으로 보기 버튼 표기 (앱 스킴 실행, 미설치 시 스토어로 이동) <br/> <font color="red"> 버그는 아니지만, Android(스토어) /ios(디폴트) 다르게 작동함 - 운영도 동일  </font> </td>
            </tr>
    ...
    <script>
        ...
        function getAppKey(type) {
          switch(type) {
            case 'none':
              return '606a24da0860894315a72643b445fa5e';
            case 'web_only':
              return '4d1b118fb2fe91d80bf1477bf4c4b4cb';
            case 'android_only':
              return '7d4a0d9be0d08d39623b14b0ceb59d13';
            case 'all':
              return '4a11515f6f6e325fcc617f54c2cbd17c';
          }
        }
        // SDK 초기화 함수
        ...
        function loginWithKakao(type) {
            window.location.href = 'https://alpha-kauth.kakao.com/oauth/authorize?client_id=' + getAppKey(type) + '&redirect_uri=<?php echo $redirect_uri;?>&response_type=code&state='+type;
        }
